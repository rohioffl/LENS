#!/usr/bin/env python3
"""EKS → GKE Terraform helper inspired by terraform-vpc.py and terraform-ecs2gke.py."""
import argparse
import json
import math
import os
...
class EksNodegroupDetail:
    ...
    def summary_line(self) -> str:
        instance_desc = ",".join(self.instance_types) or "unspecified"
        subnet_desc = ",".join(self.subnets) or "none"
        return (
            f"nodegroup={self.name} instances={instance_desc} desired={self.desired_count} "
            f"min={self.min_size} max={self.max_size} per_node≈{self.cpu_vcpu:.2f} vCPU/"
            f"{self.memory_mb} MB subnets={subnet_desc}"
        )
    ...
    def to_payload(self) -> Dict[str, Any]:
        return {
            ...
            "cpu_vcpu": round(self.cpu_vcpu, 2),
...
def sanitize_name(value: str, default: str = "eks-cluster") -> str:
...
def sanitize_label_value(value: str) -> str:
    value = value or "eks-cluster"
    cleaned = re.sub(r"[^a-z0-9_-]", "-", value.lower())
    cleaned = cleaned.strip("-")
    return cleaned or "eks-cluster"
    ...
    try:
        ...
        if "ClusterNotFoundException" in stderr:
            print(
                ...
                "   Verify the EKS cluster name/region and credentials."
            )
        else:
            print(
...
def describe_eks_cluster(cluster_name: str, region: str) -> Dict[str, Any]:
...
def describe_instance_capacities(region: str, instance_types: Iterable[str]) -> Dict[str, Tuple[float, int]]:
    types = [t for t in instance_types if t]
    if not types:
        return {}
    cmd = [
        "aws",
        ...
        *types,
        "--query",
        "InstanceTypes[*].{Type:InstanceType,Vcpu:VCpuInfo.DefaultVCpus,Mem:MemoryInfo.SizeInMiB}",
        "--output",
        "json",
    ]
    raw = run_aws_cli(cmd)
    ...
    for entry in data:
        try:
            cpu = float(entry.get("Vcpu", 0))
            mem = safe_int(entry.get("Mem"))
            result[entry.get("Type", "")] = (cpu, mem)
        except Exception:
            continue
    return result


def collect_eks_nodegroups(cluster_name: str, region: str) -> Tuple[Dict[str, Any], List[EksNodegroupDetail]]:
    cluster_details = describe_eks_cluster(cluster_name, region)
    if not cluster_details:
        return {}, []

    ng_list_raw = run_aws_cli([
        "aws",
        "eks",
        "list-nodegroups",
        "--cluster-name",
    ...
    nodegroup_names = json.loads(ng_list_raw).get("nodegroups", [])
    nodegroups: List[EksNodegroupDetail] = []
    for ng_name in nodegroup_names:
        print(f"➡️ Processing nodegroup: {ng_name}")
        ng_desc_raw = run_aws_cli([
            "aws",
            "eks",
            "describe-nodegroup",
            "--cluster-name",
        ...
        if not ng_desc_raw:
            continue
        ng_data = json.loads(ng_desc_raw).get("nodegroup")
        if not ng_data:
            continue
        scaling = ng_data.get("scalingConfig", {}) or {}
        desired = max(safe_int(scaling.get("desiredSize")), 0)
        min_size = max(safe_int(scaling.get("minSize")), 0)
        max_size = max(safe_int(scaling.get("maxSize")), desired)
        instance_types = ng_data.get("instanceTypes") or []
        capacity_map = describe_instance_capacities(region, instance_types)
        instance_cpu = 0.0
        instance_mem = 0
        chosen_type = instance_types[0] if instance_types else None
        if chosen_type and chosen_type in capacity_map:
            instance_cpu, instance_mem = capacity_map[chosen_type]
        elif capacity_map:
            any_type = next(iter(capacity_map.values()))
            instance_cpu, instance_mem = any_type
        nodegroups.append(EksNodegroupDetail(
            name=ng_name,
            nodegroup=ng_data,
            ...
            min_size=min_size,
            max_size=max_size,
            cpu_vcpu=instance_cpu,
            memory_mb=instance_mem,
            subnets=ng_data.get("subnets", []) or [],
        ))
    return cluster_details, nodegroups


def list_subnets_with_names(region: str, subnet_ids: Iterable[str]) -> Dict[str, Dict[str, Optional[str]]]:
...
def list_eks_clusters(region: str) -> List[str]:
    response = run_aws_cli([
        "aws",
        "eks",
        "list-clusters",
    ...
    return sorted({c for c in clusters if isinstance(c, str)})


def aggregate_nodegroup_resources(nodegroups: Sequence[EksNodegroupDetail]) -> Tuple[float, int, float, int, int]:
    total_cpu_vcpu = sum(ng.cpu_vcpu * ng.desired_count for ng in nodegroups)
    total_memory_mb = sum(ng.memory_mb * ng.desired_count for ng in nodegroups)
    max_task_cpu_vcpu = max((ng.cpu_vcpu for ng in nodegroups), default=0.0)
    max_task_mem_mb = max((ng.memory_mb for ng in nodegroups), default=0)
    total_desired = sum(ng.desired_count for ng in nodegroups)
    return total_cpu_vcpu, total_memory_mb, max_task_cpu_vcpu, max_task_mem_mb, total_desired


def build_nodegroup_summary(nodegroups: Sequence[EksNodegroupDetail], total_cpu_vcpu: float, total_memory_mb: int,
                            total_desired: int) -> str:
    if not nodegroups:
        return "No nodegroups discovered."
    header = (
        f"Total nodegroups: {len(nodegroups)}\n"
        f"Total desired nodes: {total_desired}\n"
        f"Approx capacity (desired): {total_cpu_vcpu:.2f} vCPU / {total_memory_mb} MB\n"
    )
    lines = [header, "Per-nodegroup details:"]
    for ng in nodegroups:
        lines.append("- " + ng.summary_line())
    return "\n".join(lines)
...
def build_gke_plan(cluster_name: str, aws_region: str, args: argparse.Namespace,
                   nodegroups: Sequence[EksNodegroupDetail], resolved_location: Optional[str],
                   location_note: Optional[str]) -> Tuple[Dict[str, Any], Optional[str]]:
    total_cpu_vcpu, total_memory_mb, max_task_cpu_vcpu, max_task_mem_mb, total_desired = aggregate_nodegroup_resources(nodegroups)
    ...
    plan = {
        ...
        "workloads_total_nodes": total_desired,
...
def build_plan_recommendations(plan: Dict[str, Any], nodegroups: Sequence[EksNodegroupDetail]) -> List[str]:
    if not nodegroups:
        return []

    total_services = len(nodegroups)
    total_tasks = plan.get("workloads_total_nodes") or 0
    ...
    recommendations.append(
        f"Plan covers {total_services} nodegroups (~{total_tasks} desired nodes); align workload scaling with the "
    ...
    max_task_cpu = max((svc.cpu_vcpu for svc in nodegroups), default=0.0)
    max_task_mem = max((svc.memory_mb for svc in nodegroups), default=0)
    if node_cpu and max_task_cpu > node_cpu:
        recommendations.append(
            f"Largest nodegroup type ({max_task_cpu:.2f} vCPU per node) exceeds the selected GKE node type ({node_cpu:.2f}); pick a bigger machine."
        )
    if node_mem and max_task_mem > node_mem:
        recommendations.append(
            f"Largest nodegroup type ({format_memory(max_task_mem)} per node) exceeds the selected GKE node memory ({format_memory(node_mem)}); adjust machine sizing."
        )

    cpu_sorted = sorted(
        (svc for svc in nodegroups if svc.cpu_vcpu > 0 and svc.desired_count > 0),
    ...
    if cpu_sorted:
        ...
        recommendations.append("CPU-heavy nodegroups: " + ", ".join(cpu_entries))

    mem_sorted = sorted(
        (svc for svc in nodegroups if svc.memory_mb > 0 and svc.desired_count > 0),
    ...
    if mem_sorted:
        ...
        recommendations.append("Memory-heavy nodegroups: " + ", ".join(mem_entries))
...
def deterministic_terraform_bundle(plan: Dict[str, Any], cluster_name: str) -> Dict[str, str]:
    node_pool_defaults = {
        ...
        "labels": {
            "source": "eks-migration",
            "eks_cluster": sanitize_label_value(cluster_name),
        },
        "tags": ["eks-migration"],
    ...
    def _extract_region_from_location_str(loc: Optional[str]) -> Optional[str]:
        if not loc:
            return None
        if loc.count("-") >= 2:
    ...
    region_value = _extract_region_from_location_str(plan.get("location"))
    ...
    tfvars = [
        ...
        f"region = {format_hcl_value(region_value)}",
...
def prompt_eks_subnets_and_create_nodepools(
    region: str,
    nodegroups: Sequence[EksNodegroupDetail],
    gcp_location: Optional[str],
    existing_node_locations: Optional[Sequence[str]] = None,
    default_gcp_subnetwork: Optional[str] = None,
) -> List[Dict[str, Any]]:
    """Detect EKS nodegroup subnets and build a single node pool for GKE."""
    subnet_set = set()
    for ng in nodegroups:
        for subnet in ng.subnets:
            subnet_set.add(subnet)

    if not subnet_set:
        print("⚠️ No EKS subnets detected in nodegroups.")
        return []

    subnet_map = list_subnets_with_names(region, subnet_set)
    ordered_subnets = [(subnet_id, subnet_map[subnet_id]) for subnet_id in sorted(subnet_map.keys())]

    print("\n🕸️ EKS subnets detected across all nodegroups:")
    ...
    print(f"\nTotal unique EKS subnets: {len(subnet_map)}")
    print("Select which subnet should back the primary GKE node pool.\n")
...
def build_interactive_cli_args() -> List[str]:
    ...
    if clusters:
        ...
        cluster_idx = prompt_select(f"Select EKS cluster in {region}", cluster_prompt)
        if cluster_idx == len(clusters):
            cluster = prompt_text("EKS cluster name")
        else:
            cluster = clusters[cluster_idx]
    else:
        print(f"⚠️ No EKS clusters discovered in {region} (or insufficient permissions).")
        cluster = prompt_text("EKS cluster name")
...
def display_plan(cluster_name: str, plan: Dict[str, Any], nodegroups: Sequence[EksNodegroupDetail],
    ...
    if location_note:
        print("\nℹ️ " + location_note)
    print(
        "\n📊 Estimated EKS footprint (based on nodegroup desired counts): "
...
def ensure_generate_args(args: argparse.Namespace) -> None:
    if not args.cluster:
        args.cluster = prompt_text("EKS cluster name")
    if not args.region:
...
def run_generate_terraform_mode(args: argparse.Namespace) -> None:
    ...
    cluster_details, nodegroups = collect_eks_nodegroups(args.cluster, args.region)
    if not cluster_details:
        print("⚠️ Could not retrieve EKS cluster metadata; aborting Terraform generation.")
        return
    if not nodegroups:
        print("⚠️ Unable to retrieve any EKS nodegroups; nothing to model.")
        return
    ...
    node_pools = prompt_eks_subnets_and_create_nodepools(
        args.region,
        nodegroups,
        location_hint,
    ...
    total_cpu_vcpu, total_memory_mb, _, _, total_desired = aggregate_nodegroup_resources(nodegroups)
    summary = build_nodegroup_summary(nodegroups, total_cpu_vcpu, total_memory_mb, total_desired)
    resolved_location, location_note = resolve_gcp_location(args.region, args.gcp_location)
    plan, _ = build_gke_plan(args.cluster, args.region, args, nodegroups, resolved_location, location_note)
    recommendations = build_plan_recommendations(plan, nodegroups)
    display_plan(args.cluster, plan, nodegroups, summary, recommendations)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="EKS → GKE Terraform toolkit (inspired by terraform-vpc.py). Safe dry-run by default.")
    ...
    tf_parser = sub.add_parser("generate-terraform", help="Generate Terraform configuration for a GKE cluster sized from an EKS footprint.")
