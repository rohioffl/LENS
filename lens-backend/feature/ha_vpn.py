...
try:  # Allow running as package or standalone script
    ...
    from .gcp_vpn import GcpVpnError, compute_covering_cidr, get_gcp_network
except ImportError:  # pragma: no cover - fallback for direct script execution
    import terraform_vpc as vpc_mod  # type: ignore
    from gcp_vpn import GcpVpnError, compute_covering_cidr, get_gcp_network  # type: ignore
...
def _load_service_account_info(raw_value: Optional[str]) -> Dict[str, Any]:
...
# ---------------------------------------------------------------------------
# Planning helpers for Django API/UI (non-mutating)
# ---------------------------------------------------------------------------


class HaVpnPlanError(GcpVpnError):
    """Raised when HA VPN planning fails."""


def _plan_placeholder_cidr(subnets: List[Dict[str, str]]) -> str:
    cidrs = [s.get("cidr") for s in subnets if s.get("cidr")]
    if not cidrs:
        return "10.0.0.0/24"
    try:
        return compute_covering_cidr(cidrs)
    except GcpVpnError:
        return cidrs[0]
...
def _plan_build_context(
    ...
    prefix = sanitize_name(name_prefix or f"ha-{aws_vpc.id}-{gcp_region}")
    names = build_resource_names(prefix)
    placeholder_cidr = _plan_placeholder_cidr(gcp_network.get("subnetworks", []))
    return {
        "aws": {
            "id": aws_vpc.id,
            "cidr": aws_vpc.cidr,
        ...
        "vpn": {**names, "placeholder_cidr": placeholder_cidr, "name_prefix": prefix},
    }


def _plan_render_markdown(context: Dict[str, Any]) -> str:
    aws = context["aws"]
    gcp = context["gcp"]
    vpn = context["vpn"]
    first_gcp_cidr = gcp["selected_subnets"][0].get("cidr") if gcp["selected_subnets"] else vpn["placeholder_cidr"]
...
{''.join([f"      - {entry['name']} ({entry.get('cidr') or 'unknown'}) @ {entry['region']}\n" for entry in gcp['selected_subnets']] ) or '      - (all)\n'}
        ...
        ## Suggested Resource Names (prefix: `{vpn['name_prefix']}`)
        - AWS VGW: `{vpn['aws_vgw']}`
        - AWS Customer Gateways: `{vpn['aws_cgw_0']}`, `{vpn['aws_cgw_1']}`
        - AWS VPN Connections: `{vpn['aws_vpn_0']}`, `{vpn['aws_vpn_1']}`
        - GCP Cloud Router: `{vpn['gcp_router']}`
        - GCP HA VPN Gateway: `{vpn['gcp_ha_gateway']}`
        - GCP External VPN Gateway: `{vpn['gcp_peer_gateway']}`
        - VPN Tunnels: `{vpn['gcp_tunnel_prefix']}-1` ... `{vpn['gcp_tunnel_prefix']}-4`

        ## High-level Steps
        ### GCP
        1. Create Cloud Router `{vpn['gcp_router']}` (ASN {gcp['asn']}) on network **{gcp['name']}** in **{gcp['region']}**.
        2. Create HA VPN Gateway `{vpn['gcp_ha_gateway']}` (2 interfaces with external IPs).
        3. Create External VPN Gateway `{vpn['gcp_peer_gateway']}` representing the AWS VGW (4 interfaces).
        4. Create 4 tunnels `{vpn['gcp_tunnel_prefix']}-[1-4]` pointing to AWS outside IPs; configure BGP with link-local IPs.

        ### AWS
        1. Create Virtual Private Gateway `{vpn['aws_vgw']}` (ASN {aws['asn']}) and attach to VPC **{aws['id']}**.
        2. Create two Customer Gateways `{vpn['aws_cgw_0']}` / `{vpn['aws_cgw_1']}` targeting the GCP HA VPN interface IPs.
        3. Create two Site-to-Site VPN connections `{vpn['aws_vpn_0']}` / `{vpn['aws_vpn_1']}` (dynamic/BGP).
        4. Enable VGW route propagation or update route tables for `{aws['cidr']}` / `{first_gcp_cidr}`.

        ## Notes
        - HA VPN uses IKEv2 and BGP; ensure CIDR advertisements cover your selected subnets (placeholder `{vpn['placeholder_cidr']}`).
        - Fill in the four GCP/AWS tunnel outside IPs and BGP link-local ranges once allocated.
        """
    ).strip() + "\n"


def _plan_render_sample(context: Dict[str, Any]) -> str:
    aws = context["aws"]
    gcp = context["gcp"]
    vpn = context["vpn"]
    return textwrap.dedent(
        f"""
        # Terraform-like snippet (replace placeholders)

        resource "aws_vpn_gateway" "{vpn['aws_vgw']}" {{
          vpc_id = "{aws['id']}"
          amazon_side_asn = {aws['asn']}
          tags = {{ Name = "{vpn['aws_vgw']}" }}
        }}

        resource "google_compute_router" "{vpn['gcp_router']}" {{
          name    = "{vpn['gcp_router']}"
          network = "{gcp['name']}"
          region  = "{gcp['region']}"
          bgp {{ asn = {gcp['asn']} }}
        ...
        # Continue with HA VPN gateways, external gateways, tunnels, and BGP sessions
        # using the generated names as a baseline.
        """
    ).strip() + "\n"


def generate_ha_vpn_artifacts(
    access_key: str,
    secret_key: str,
    aws_region: str,
    aws_vpc_id: str,
    ...
    plan_md = _plan_render_markdown(context)
    sample_tf = _plan_render_sample(context)
    metadata = json.dumps(context, indent=2)
    return [
        {"filename": "ha_vpn_plan.md", "content": plan_md.encode("utf-8"), "content_type": "text/markdown"},
        {"filename": "ha_vpn_samples.tf", "content": sample_tf.encode("utf-8"), "content_type": "text/plain"},
        {"filename": "ha_vpn_context.json", "content": metadata.encode("utf-8"), "content_type": "application/json"},
    ]
...
def get_gcp_compute_service(service_key: Optional[str] = None):
    """Initialize GCP Compute API client"""
    if service_key:
        info = _load_service_account_info(service_key)
        creds = service_account.Credentials.from_service_account_info(info, scopes=SCOPES)
    else:
        key_path = os.environ.get('GOOGLE_APPLICATION_CREDENTIALS')
        if not key_path:
            raise GcpVpnError(
                "GOOGLE_APPLICATION_CREDENTIALS must be set or a GCP service key must be provided."
            )
...
def setup_ha_vpn(config: HAVPNConfig, prefix: str, service_key: Optional[str] = None):
    ...
    compute = get_gcp_compute_service(service_key)
